...
(function (W, D) {
    ...
    var CFG = {
         ...
         * FIX #1: добавлены address, pre, summary.
         */
        TEXT_TAGS : {
         ...
         * FIX #1: полный HTML5; FIX #2: <a> расширен, добавлен rel.
         * FIX #2: <button> убран из TEXT_TAGS, добавлен в ATTR_MAP+ATTR_HAS_TEXT.
         */
        ATTR_MAP : {
        ...
        /*
         * Теги, у которых есть и innerHTML-текст, и атрибуты → тип 'both'.
         * FIX #2: добавлен 'a', 'button'.
         */
    ...
    function histSave(db, key, stack, pos) {
        ...
        db.transaction(CFG.HISTORY_STORE, 'readwrite')
          ...
          .put({ key: key, stack: stack, pos: pos,
     ...
     *
     * FIX #2 / #3: если тег находится в ATTR_MAP И в ATTR_HAS_TEXT
     * (т.е. это <a>, <button>…), он получает тип 'both' когда имеет
     * текстовое содержимое, и тип 'attrs' когда пустой.
     * Это гарантирует что <a>текст</a> всегда показывает textarea.
     */
    function walkMark(node, depth) {
        ...
        if (isAttr) {
            if (!hasEditableDescendant(node)) {
                ...
                /*
                 * FIX #2/#3: тег с текстом (a, button, time…) → 'both',
                 * иначе только атрибуты.
                 */
                var hasBody = !!CFG.ATTR_HAS_TEXT[tag] &&
                              (node.textContent || '').trim().length > 0;
                S.map[id2] = { node: node, tag: tag,
    ...
    function buildFmtBar() {
        ...
        /* FIX #1: <small> полностью поддерживается в HTML5 (fine print) */
        ...
        (function() {
            ...
            var b  = mk('button', { type:'button', cls:'sev8-fb', title:'Ссылка — &lt;a href&gt;',
        ...
        (function() {
            var w2  = mk('div', { cls:'sev8-fbwrap' });
            var b   = mk('button', { type:'button', cls:'sev8-fb',
    ...
    /* FIX #2: target рисуется как <select> */
    var TARGET_OPTIONS = [
        ['', '— не задан —'],
    ...
    function buildAttrInput(attr, curValue) {
        ...
        if (attr === 'target') {
            ...
            inp = mk('input', { type:'text', cls:'sev8-in',
                css:'font-family:monospace;font-size:11px;min-width:0;',
                value: curValue });
        }
     ...
     * FIX #2: всегда показывает href/target для <a>, даже если атрибут отсутствует.
     */
    ...
    /* ════════ ТУЛБАР ════════ */
    /*
     * FIX #4: убраны 4 паразитных tb.appendChild() до создания topRow.
     * Все группы сразу вставляются в topRow.
     */
    function buildToolbar(onMode) {
        ...
        var undoBtn = mk('button', { cls:'sev8-btn', type:'button',
            title:'Отменить (Ctrl+Z)', html:'↩ Отмена' });
        var redoBtn = mk('button', { cls:'sev8-btn', type:'button',
        ...
        var syncBtn = mk('button', { cls:'sev8-btn primary', type:'button',
        ...
        var fsBtn = mk('button', { cls:'sev8-btn', type:'button',
        ...
        if (S.aceMode && S.aceEl) {
            var backBtn = mk('button', { cls:'sev8-btn', type:'button',
            ...
            var backBtn2 = mk('button', { cls:'sev8-btn', type:'button',
    ...
    function renderTreeChildren(node, container, depth) {
        Array.prototype.forEach.call(node.children, function(child) {
            ...
            var arrow  = mk('span', { cls:'sev8-ta' + (isContainer ? ' open' : ''),
                                      txt: isContainer ? '▶' : '·' });
    ...
    /* FIX #2: buildLeafFields показывает «Текст ссылки:» для <a> */
    ...
    function buildStruct() {
        var wrap = mk('div', { id:'sev8-struct',
    ...
    /*
     * FIX #3: orphan-элементы (элементы вне section/header/…)
     * теперь отображаются в отдельной карточке «Прочее содержимое».
     */
    ...
    /* FIX #2: buildRow показывает «Текст ссылки:» и select для target */
    function buildRow(info, id) {
        ...
        if (/^(li|p|dt|dd|tr|option|blockquote)$/.test(tag)) {
            var aBtn = mk('button', { cls:'sev8-mb add', type:'button', txt:'+',
    ...
    function structAdd(id) {
        ...
        var defs = { li:'Новый пункт', p:'Новый абзац', dt:'Термин',
    ...
    function treeGroupAdd(node, tag) {
        var p = node.parentNode; if (!p) return;
        var defs = { li:'Новый пункт', dt:'Термин', dd:'Описание',
    ...
    function buildVisual() {
        ...
        wrap.appendChild(mk('div', { cls:'sev8-bar info',
    ...
    /* FIX #2: оверлей для <a> показывает textarea текста + поля href/target/rel */
    function showOvl(fr, iEl, info, id) {
        ...
        if (info.type === 'text' || info.type === 'both') {
            if (info.tag === 'a')
                ovl.appendChild(mk('div', { cls:'sev8-sub-lbl',
    ...
    function buildSource() {
        ...
        bar.appendChild(mk('span', { cls:'sev8-bar-txt',
    ...
    function mount(rawHtml) {
        ...
        D.addEventListener('keydown', function(e) {
            ...
            if ((e.ctrlKey || e.metaKey) && !e.shiftKey && e.key === 'z' && inEd)
                { e.preventDefault(); histUndo(); }
            if ((e.ctrlKey || e.metaKey) && (e.key === 'y' || (e.shiftKey && e.key === 'z')) && inEd)
                { e.preventDefault(); histRedo(); }
        }, false);
