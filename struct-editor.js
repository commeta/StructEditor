...
(function (W, D) {
    ...
    /* ════════ ЦВЕТА — ExtJS3 / MODX Revolution 2.8 ════════ */
    var C = {
    ...
    var CFG = {
        ...
        TEXT_TAGS    : {
            ...
            th:1,td:1,caption:1,button:1,label:1
        },
        ATTR_MAP     : {
            a      : ['href','title','target'],
        ...
        LEAF_CONTAINERS: { div:1, span:1 },
        LEAF_MIN_CH  : 2,
        SKIP : {
            script:1,style:1,meta:1,link:1,noscript:1,
            br:1,hr:1,wbr:1,svg:1,input:1,select:1,
            textarea:1,option:1,optgroup:1,template:1,'sev8-ph':1
        },
        MAX_DEPTH    : 40,
    ...
    /* ════════ УТИЛИТЫ ════════ */
    var L = W.console ? function(m){ console.log('[SEv14] ' + m); } : function(){};
    ...
    function histSave(db, key, stack, pos) {
        ...
        db.transaction(CFG.HISTORY_STORE, 'readwrite')
          ...
          .put({ key: key, stack: stack, pos: pos, hash: simpleHash(stack[stack.length - 1]) });
    }
    ...
    function tokenize(html) {
        ...
        TOKEN_PATTERNS.forEach(function(re) {
            ...
            while ((m = re.exec(html)) !== null) {
                var overlap = false;
                for (var i = m.index; i < m.index + m[0].length; i++) if (used[i]) { overlap = true; break; }
                if (!overlap) {
        ...
        for (var i = matches.length - 1; i >= 0; i--) {
            var match = matches[i];
            result = result.substring(0, match.index) + '\x00SEVPH' + i + '\x00' + result.substring(match.index + match.length);
        }
    ...
    function detokenize(html) {
        ...
        var pat = '<div[^>]*\\b' + CFG.PH_ATTR + '="(\\d+)"[^>]*>\\s*<\\/div>|<div[^>]*\\b' + CFG.PH_ATTR + '="(\\d+)"[^>]*/>';
    ...
    function hasEditableDescendant(node) {
        for (var i = 0; i < node.children.length; i++) {
            ...
            if (CFG.LEAF_CONTAINERS[ct] && (c.textContent || '').trim().length >= CFG.LEAF_MIN_CH) return true;
    ...
    function walkMark(node, depth) {
        ...
        if (isText) {
            ...
            if (!hasTextKid) {
                ...
                S.map[id] = { node: node, tag: tag, type: isAttr ? 'both' : 'text', attrs: isAttr ? CFG.ATTR_MAP[tag] : null };
                return;
            }
        ...
        if (isAttr) {
            if (!hasEditableDescendant(node)) {
                ...
                S.map[id2] = { node: node, tag: tag, type: 'attrs', attrs: CFG.ATTR_MAP[tag] };
                return;
            }
    ...
    function serialize() {
        ...
        Array.prototype.forEach.call(clone.querySelectorAll('*'), function(n) {
            Array.prototype.slice.call(n.attributes).forEach(function(a) {
                if (/^data-sev/.test(a.name) && a.name !== CFG.PH_ATTR) n.removeAttribute(a.name);
            });
        });
    ...
    function findNativeTextarea() {
        for (var i = 0; i < CFG.TA_SELECTORS.length; i++) {
            ...
            try {
                ...
                if (el && el.tagName === 'TEXTAREA') {
                    L('Нативный textarea найден: selector="' + sel + '" id="' + el.id + '"');
                    return el;
                }
                if (el) {
                    var ta = el.tagName === 'TEXTAREA' ? el : el.querySelector('textarea');
                    if (ta) {
                        L('Нативный textarea (в контейнере): id="' + ta.id + '"');
                        return ta;
                    }
                }
            } catch(e) {}
        ...
        for (var j = 0; j < all.length; j++) {
            var t = all[j];
            var rows = parseInt(t.getAttribute('rows') || '0', 10);
            if (rows >= 10 || t.value.length > 50 || t.classList.contains('x-form-textarea')) {
    ...
    function findSections(body) {
        ...
        (function walk(n) {
            ...
            if (/^(section|article|header|footer|main|nav|aside)$/.test(n.tagName.toLowerCase())) { secs.push(n); return; }
    ...
    function secTitle(n) {
        ...
        if (h) return (h.textContent || '').trim().substring(0, 50) || '<' + n.tagName.toLowerCase() + '>';
        var cls = (n.className || '').trim().split(/\s+/).filter(function(c) { return c.length > 1; }).slice(0, 2).join(' ');
    ...
    function prevElSib(n) { var s = n.previousSibling; while (s && s.nodeType !== 1) s = s.previousSibling; return s; }
    function nextElSib(n) { var s = n.nextSibling; while (s && s.nodeType !== 1) s = s.nextSibling; return s; }
    ...
    function injectCSS() {
        ...
        s.textContent = [
            ...
            '.sev8-tb-group:last-child{border-right:none;margin-left:auto;}',
            ...
            '.sev8-al{font-size:11px;color:', C.textMuted, ';font-family:monospace;flex-shrink:0;min-width:30px;}',
            ...
            /* Дерево — компактные отступы: 4px на уровень вместо 8px */
            ...
            /* Ключевое изменение: ширина направляющей 10px и отступ 4px вместо 16px/8px */
            ...
            '.sev8-leaf-body{padding:3px 3px 5px 16px;}.sev8-leaf-body .sev8-in{font-size:12px;}',
            ...
            '#sev8-ovl-atlbl{font-size:11px;color:', C.textMuted, ';font-family:monospace;flex-shrink:0;min-width:42px;}',
    ...
    function toggleFullscreen(btn) {
        if (!S.fsMode) {
            ...
            S.rootEl.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;width:100vw;height:100vh !important;z-index:99998;margin:0;border-radius:0;display:flex;flex-direction:column;';
            D.body.style.overflow = 'hidden'; S.fsMode = true; btn.textContent = '⊞ Свернуть';
        } else {
            S.rootEl.style.cssText = S.fsOrigCss; D.body.style.overflow = '';
    ...
    function buildFmtBar() {
        ...
        function clearFmt() {
            ...
            var inl = 'strong|em|u|s|del|ins|mark|code|small|sub|sup|q|abbr|span|b|i|cite|kbd|samp|var';
            var cleaned = sel.replace(new RegExp('<(?:' + inl + ')(?:\\s[^>]*)?>', 'gi'), '')
        ...
        function btnDropdown(label, title, items) {
            ...
            b.addEventListener('click', function(e) {
                e.stopPropagation();
                Array.prototype.forEach.call(D.querySelectorAll('.sev8-fbdd.open'), function(el) { if (el !== dd) el.classList.remove('open'); });
        ...
        function colorBtn(sym, title, applyFn) {
            ...
            var picker = mk('input', { type: 'color', value: '#e06c75',
            ...
            b.addEventListener('click', function() {
                var savedTarget = S.fmtTarget;
                picker.onchange = function() {
                    clrBar.style.background = picker.value;
                    S.fmtTarget = savedTarget;
        ...
        bar.appendChild(sep());
        bar.appendChild(btn('<span style="background:#fff3cd;color:#333;padding:0 2px;border-radius:2px;font-size:11px">M</span>', 'Выделение — &lt;mark&gt;', '', function() { wrap('<mark>', '</mark>'); }));
        bar.appendChild(btn('<code style="font-size:10px;background:#f0f0f0;padding:0 2px;border-radius:2px;color:#333">&lt;/&gt;</code>', 'Код — &lt;code&gt;', '', function() { wrap('<code>', '</code>'); }));
        bar.appendChild(btn('<span style="font-style:italic;font-size:11px">&ldquo;q&rdquo;</span>', 'Цитата — &lt;q&gt;', '', function() { wrap('<q>', '</q>'); }));
        bar.appendChild(btn('<ins style="font-size:11px">+ins</ins>', 'Вставка — &lt;ins&gt;', '', function() { wrap('<ins>', '</ins>'); }));
        bar.appendChild(btn('x<sup style="font-size:8px">2</sup>', 'Надстрочный — &lt;sup&gt;', '', function() { wrap('<sup>', '</sup>'); }));
        bar.appendChild(btn('x<sub style="font-size:8px">2</sub>', 'Подстрочный — &lt;sub&gt;', '', function() { wrap('<sub>', '</sub>'); }));
        bar.appendChild(btn('<small style="font-size:9px">Aa</small>', 'Мелкий — &lt;small&gt;', '', function() { wrap('<small>', '</small>'); }));
        bar.appendChild(sep());
        bar.appendChild(colorBtn('A', 'Цвет текста', function(clr) { wrap('<span style="color:' + clr + '">', '</span>'); }));
        bar.appendChild(colorBtn('■', 'Цвет фона', function(clr) { wrap('<span style="background-color:' + clr + '">', '</span>'); }));
        ...
        (function() {
            ...
            var b = mk('button', { type: 'button', cls: 'sev8-fb', title: 'Ссылка — &lt;a href&gt;', html: '<span style="text-decoration:underline;color:' + C.accent + '">Ссылка</span>' });
            var dd = mk('div', { cls: 'sev8-fbdd' }); var pad = mk('div', { cls: 'sev8-fbdd-pad' });
            ...
            tRow.appendChild(chk); tRow.appendChild(mk('label', { 'for': 'sev8-link-blank', txt: 'Новая вкладка' }));
            ...
            applyBtn.addEventListener('click', function() {
                ...
                wrap('<a href="' + href + '"' + (chk.checked ? ' target="_blank" rel="noopener"' : '') + '>', '</a>', 'текст ссылки');
                urlInp.value = ''; dd.classList.remove('open');
            });
            urlInp.addEventListener('keydown', function(e) { if (e.key === 'Enter') { e.preventDefault(); applyBtn.click(); } if (e.key === 'Escape') dd.classList.remove('open'); });
            pad.appendChild(urlInp); pad.appendChild(tRow); pad.appendChild(applyBtn); dd.appendChild(pad);
            b.addEventListener('mousedown', function(e) { e.preventDefault(); });
            b.addEventListener('click', function(e) {
                e.stopPropagation();
                Array.prototype.forEach.call(D.querySelectorAll('.sev8-fbdd.open'), function(el) { if (el !== dd) el.classList.remove('open'); });
        ...
        (function() {
            var w2 = mk('div', { cls: 'sev8-fbwrap' });
            var b = mk('button', { type: 'button', cls: 'sev8-fb', title: 'Аббревиатура — &lt;abbr title&gt;', html: '<span style="border-bottom:1px dotted ' + C.textMuted + ';font-size:10px">ABR</span>' });
            var dd = mk('div', { cls: 'sev8-fbdd' }); var pad = mk('div', { cls: 'sev8-fbdd-pad' });
            ...
            applyBtn.addEventListener('mousedown', function(e) { e.preventDefault(); });
            applyBtn.addEventListener('click', function() { var t = inp.value.trim(); if (!t) return; wrap('<abbr title="' + t + '">', '</abbr>'); inp.value = ''; dd.classList.remove('open'); });
            inp.addEventListener('keydown', function(e) { if (e.key === 'Enter') { e.preventDefault(); applyBtn.click(); } if (e.key === 'Escape') dd.classList.remove('open'); });
        ...
        (function() {
            var w2 = mk('div', { cls: 'sev8-fbwrap' });
            var b = mk('button', { type: 'button', cls: 'sev8-fb', title: 'Обернуть в &lt;span class&gt;', html: '<span style="font-family:monospace;font-size:10px">.cls</span>' });
            var dd = mk('div', { cls: 'sev8-fbdd' }); var pad = mk('div', { cls: 'sev8-fbdd-pad' });
            ...
            applyBtn.addEventListener('mousedown', function(e) { e.preventDefault(); });
            applyBtn.addEventListener('click', function() { var c = inp.value.trim().replace(/[^a-zA-Z0-9_\- ]/g, ''); if (!c) return; wrap('<span class="' + c + '">', '</span>'); inp.value = ''; dd.classList.remove('open'); });
            inp.addEventListener('keydown', function(e) { if (e.key === 'Enter') { e.preventDefault(); applyBtn.click(); } if (e.key === 'Escape') dd.classList.remove('open'); });
            pad.appendChild(inp); pad.appendChild(applyBtn); dd.appendChild(pad);
            b.addEventListener('mousedown', function(e) { e.preventDefault(); });
            b.addEventListener('click', function(e) {
        ...
        D.addEventListener('click', function() {
            Array.prototype.forEach.call(D.querySelectorAll('.sev8-fbdd.open'), function(el) { el.classList.remove('open'); });
        });
        return bar;
    }
    ...
    function buildToolbar(onMode) {
        ...
        var undoBtn = mk('button', { cls: 'sev8-btn', type: 'button', title: 'Отменить (Ctrl+Z)', html: '↩ Отмена' });
        var redoBtn = mk('button', { cls: 'sev8-btn', type: 'button', title: 'Повторить (Ctrl+Y)', html: '↪ Повтор' });
        ...
        hg.appendChild(undoBtn); hg.appendChild(redoBtn); tb.appendChild(hg);

        /* Действия */
        var ag = mk('div', { cls: 'sev8-tb-group' });
        var syncBtn = mk('button', { cls: 'sev8-btn primary', type: 'button', title: 'Синхронизировать с MODX', html: '💾 Синхр.' });
        ...
        var fsBtn = mk('button', { cls: 'sev8-btn', type: 'button', title: 'Весь экран', html: '⊞ Весь экран' });
        ...
        if (S.aceMode && S.aceEl) {
            backBtn = mk('button', { cls: 'sev8-btn', type: 'button', title: 'Вернуться к ACE-редактору', html: '↩ ACE' });
            ...
            backBtn = mk('button', { cls: 'sev8-btn', type: 'button', title: 'Вернуться к нативному редактору MODX', html: '✎ Textarea' });
            backBtn.onclick = function() {
                syncNow(); closeOvl();
                if (S.fsMode) toggleFullscreen(fsBtn);
                if (S.rootEl) S.rootEl.style.display = 'none';
        ...
        if (backBtn) ag.appendChild(backBtn);
    ...
    /* ════════ ДЕРЕВО ════════ */
    var TREE_CONTAINERS = { div:1, section:1, article:1, header:1, footer:1, main:1, nav:1, aside:1, ul:1, ol:1, dl:1, table:1, thead:1, tbody:1, tfoot:1, tr:1, figure:1, form:1, fieldset:1, blockquote:1, span:1 };
    var TREE_SKIP = { script:1, style:1, meta:1, link:1, noscript:1, br:1, hr:1, wbr:1, template:1 };
    ...
    function renderTreeChildren(node, container, depth) {
        Array.prototype.forEach.call(node.children, function(child) {
            ...
            var hasContent = isLeaf || child.querySelector('[' + CFG.MARK + ']') || !!(child.children.length);
            ...
            var arrow = mk('span', { cls: 'sev8-ta' + (isContainer ? ' open' : ''), txt: isContainer ? '▶' : '·' });
            ...
            var cls2 = (child.className || '').trim().split(/\s+/).filter(function(c) { return c.length > 1; }).slice(0, 2).join(' ');
            if (isLeaf) {
                var txt2 = (child.textContent || '').trim().replace(/\s+/g, ' ').substring(0, 50);
                descEl.innerHTML = (cls2 ? '<b>.' + cls2.split(' ')[0] + '</b> ' : '') + (txt2 || '—');
            } else {
                descEl.innerHTML = (cls2 ? '<b>.' + cls2.split(' ')[0] + '</b> ' : '') + child.children.length + ' эл.';
            }
            nhEl.appendChild(descEl);
            var gmEl = mk('div', { cls: 'sev8-gmicros' });
            var clBtn = mk('button', { cls: 'sev8-gm', type: 'button', txt: '⎘', title: 'Клонировать' });
            clBtn.onclick = function(e) { e.stopPropagation(); treeGroupClone(child); }; gmEl.appendChild(clBtn);
            if (/^(li|tr|dt|dd|p|blockquote|option|article|section)$/.test(tag)) {
                ...
                aBtn.onclick = function(e) { e.stopPropagation(); treeGroupAdd(child, tag); }; gmEl.appendChild(aBtn);
            }
            var uBtn = mk('button', { cls: 'sev8-gm', type: 'button', txt: '↑', title: 'Вверх' });
            uBtn.onclick = function(e) { e.stopPropagation(); treeGroupMove(child, -1); }; gmEl.appendChild(uBtn);
            var dBtn = mk('button', { cls: 'sev8-gm', type: 'button', txt: '↓', title: 'Вниз' });
            dBtn.onclick = function(e) { e.stopPropagation(); treeGroupMove(child, 1); }; gmEl.appendChild(dBtn);
            var delBtn = mk('button', { cls: 'sev8-gm del', type: 'button', txt: '✕', title: 'Удалить' });
            delBtn.onclick = function(e) { e.stopPropagation(); if (!W.confirm('Удалить <' + tag + '>?')) return; treeGroupDelete(child); }; gmEl.appendChild(delBtn);
            nhEl.appendChild(gmEl); tnEl.appendChild(nhEl);
            var ncEl = mk('div', { cls: 'sev8-nc' });
            if (isLeaf) {
                ncEl.appendChild(buildLeafFields(markId, S.map[markId]));
                ncEl.style.display = 'none';
                nhEl.addEventListener('click', function() { ncEl.style.display = ncEl.style.display !== 'none' ? 'none' : ''; });
            } else if (isContainer) {
                renderTreeChildren(child, ncEl, depth + 1);
                var open = depth < 1; ncEl.style.display = open ? '' : 'none';
                ...
                nhEl.addEventListener('click', function(e) {
                    ...
                    ncEl.style.display = isOpen ? 'none' : ''; arrow.classList.toggle('open', !isOpen);
                });
            } else { renderTreeChildren(child, ncEl, depth + 1); }
            tnEl.appendChild(ncEl); container.appendChild(tnEl);
        });
    }
    ...
    function buildLeafFields(id, info) {
        ...
        if (info.type === 'text' || info.type === 'both') {
            ...
            ta.style.minHeight = '40px'; ta.value = info.node.innerHTML;
            ta.setAttribute('data-id', id); ta.setAttribute('data-t', 'html');
            ta.addEventListener('input', debounce(function() { var r = S.map[ta.getAttribute('data-id')]; if (!r) return; r.node.innerHTML = ta.value; setStatus('● изменено'); lazySync(); }, 300));
            ta.addEventListener('change', function() { var r = S.map[ta.getAttribute('data-id')]; if (!r) return; r.node.innerHTML = ta.value; syncNow(); });
            wireFmtBar(ta); body.appendChild(ta); wrap.appendChild(body);
        }
        if ((info.type === 'attrs' || info.type === 'both') && info.attrs) {
            var aWrap = mk('div', { cls: 'sev8-leaf-arow' });
            info.attrs.forEach(function(attr) {
                if (!info.node.hasAttribute(attr)) return;
                var ai = mk('div', { cls: 'sev8-ai', css: 'flex:1;min-width:130px;' });
                ai.appendChild(mk('span', { cls: 'sev8-al', txt: attr + ':' }));
                var av = mk('input', { type: 'text', cls: 'sev8-in', css: 'font-family:monospace;font-size:11px;min-width:0;', value: info.node.getAttribute(attr) || '' });
                av.setAttribute('data-id', id); av.setAttribute('data-a', attr);
                av.addEventListener('input', debounce(function() { var r = S.map[av.getAttribute('data-id')]; if (!r) return; r.node.setAttribute(av.getAttribute('data-a'), av.value); setStatus('● изменено'); lazySync(); }, 300));
                ai.appendChild(av); aWrap.appendChild(ai);
            });
            if (aWrap.firstChild) wrap.appendChild(aWrap);
        }
        return wrap;
    }
    ...
    function buildStruct() {
        var wrap = mk('div', { id: 'sev8-struct', css: 'flex:1;display:flex;flex-direction:column;overflow:hidden;' });
        ...
        cleanBtn.onclick = cleanSpaces; bar.appendChild(cleanBtn); wrap.appendChild(bar);
    ...
    function fillCards(container) {
        ...
        if (secs.length === 1 && secs[0] === S.doc.body) {
            container.appendChild(buildCard(S.doc.body, Object.keys(S.map), 'Всё содержимое', 'body', true));
        } else {
            secs.forEach(function(sec, i) {
                var ids = Object.keys(S.map).filter(function(id) { return sec.contains(S.map[id].node); });
                container.appendChild(buildCard(sec, ids, secTitle(sec), sec.tagName.toLowerCase(), i === 0));
            });
        }
        if (!container.firstChild) {
            container.appendChild(mk('p', { css: 'color:' + C.textFaded + ';text-align:center;padding:18px;font-size:12px;', txt: 'Редактируемых элементов не найдено. Переключитесь на «Исходник».' }));
        }
    }
    ...
    function cleanSpaces() {
        ...
        Array.prototype.forEach.call(inputs, function(inp) {
            var v = inp.value;
            var cl = v.replace(/[ \t]{2,}/g, ' ').replace(/[ \t]*\n[ \t]*/g, '\n').replace(/\n{3,}/g, '\n\n').replace(/^\s+|\s+$/g, '');
            if (cl !== v) { inp.value = cl; var id = inp.getAttribute('data-id'); if (id && S.map[id]) S.map[id].node.innerHTML = cl; changed++; }
        });
    ...
    function buildCard(sec, ids, title, tag, open) {
        ...
        var body = mk('div', { cls: 'sev8-cb' }); body.style.display = open ? '' : 'none';
        if (ids.length === 0) { body.appendChild(mk('p', { css: 'color:' + C.textFaded + ';margin:0;font-size:11px;', txt: 'Нет редактируемых элементов.' })); }
        else { ids.forEach(function(id) { body.appendChild(buildRow(S.map[id], id)); }); }
        hdr.addEventListener('click', function() { open = !open; body.style.display = open ? '' : 'none'; arr.style.transform = open ? 'rotate(90deg)' : 'rotate(0deg)'; });
    ...
    function buildRow(info, id) {
        ...
        var path = nodePathStr(node); if (path) lbl.appendChild(mk('span', { cls: 'sev8-fpath', txt: '› ' + path }));
        rowHdr.appendChild(lbl);
        var micros = mk('div', { cls: 'sev8-micros' });
        var clBtn = mk('button', { cls: 'sev8-mb', type: 'button', txt: '⎘', title: 'Клонировать' }); clBtn.onclick = function() { structClone(id); }; micros.appendChild(clBtn);
        if (/^(li|p|dt|dd|tr|option|blockquote)$/.test(tag)) { var aBtn = mk('button', { cls: 'sev8-mb add', type: 'button', txt: '+', title: 'Добавить ' + tag + ' после' }); aBtn.onclick = function() { structAdd(id); }; micros.appendChild(aBtn); }
        var uBtn = mk('button', { cls: 'sev8-mb', type: 'button', txt: '↑', title: 'Вверх' }); uBtn.onclick = function() { structMove(id, -1); }; micros.appendChild(uBtn);
        var dBtn = mk('button', { cls: 'sev8-mb', type: 'button', txt: '↓', title: 'Вниз' }); dBtn.onclick = function() { structMove(id, 1); }; micros.appendChild(dBtn);
        var delBtn = mk('button', { cls: 'sev8-mb del', type: 'button', txt: '✕', title: 'Удалить' }); delBtn.onclick = function() { structDelete(id); }; micros.appendChild(delBtn);
        rowHdr.appendChild(micros); row.appendChild(rowHdr);
        if (info.type === 'text' || info.type === 'both') {
            var ta = mk('textarea', { cls: 'sev8-in' });
            ta.style.minHeight = /^(p|blockquote|li|td|th|dd|dt|div|aside|span)$/.test(tag) ? '54px' : '34px';
            ta.value = node.innerHTML; ta.setAttribute('data-id', id); ta.setAttribute('data-t', 'html');
            ta.addEventListener('input', debounce(function() { var r = S.map[ta.getAttribute('data-id')]; if (!r) return; r.node.innerHTML = ta.value; setStatus('● изменено'); lazySync(); }, 300));
            ta.addEventListener('change', function() { var r = S.map[ta.getAttribute('data-id')]; if (!r) return; r.node.innerHTML = ta.value; syncNow(); });
        ...
        if ((info.type === 'attrs' || info.type === 'both') && info.attrs) {
            ...
            info.attrs.forEach(function(attr) {
                ...
                ai.appendChild(mk('span', { cls: 'sev8-al', txt: attr + ':' }));
                var av = mk('input', { type: 'text', cls: 'sev8-in', css: 'font-family:monospace;font-size:11px;min-width:0;', value: node.getAttribute(attr) || '' });
                av.setAttribute('data-id', id); av.setAttribute('data-a', attr);
                av.addEventListener('input', debounce(function() { var r = S.map[av.getAttribute('data-id')]; if (!r) return; r.node.setAttribute(av.getAttribute('data-a'), av.value); setStatus('● изменено'); lazySync(); }, 300));
                ai.appendChild(av); aWrap.appendChild(ai);
            });
            if (aWrap.firstChild) row.appendChild(aWrap);
        }
    ...
    function structClone(id) {
        var info = S.map[id]; if (!info) return; var n = info.node, p = n.parentNode; if (!p) return;
        var cl = n.cloneNode(true); cl.removeAttribute(CFG.MARK);
        Array.prototype.forEach.call(cl.querySelectorAll('[' + CFG.MARK + ']'), function(x) { x.removeAttribute(CFG.MARK); });
    ...
    function structAdd(id) {
        ...
        var defs = { li: 'Новый пункт', p: 'Новый абзац', dt: 'Термин', dd: 'Описание', blockquote: 'Цитата', option: 'Вариант' };
        if (tag === 'tr') { newEl = n.cloneNode(true); newEl.removeAttribute(CFG.MARK); Array.prototype.forEach.call(newEl.querySelectorAll('[' + CFG.MARK + ']'), function(x) { x.removeAttribute(CFG.MARK); }); Array.prototype.forEach.call(newEl.querySelectorAll('td,th'), function(c) { c.innerHTML = ''; }); }
        else { newEl = S.doc.createElement(tag); newEl.textContent = defs[tag] || ''; }
    ...
    function treeGroupDelete(node) { var p = node.parentNode; if (!p) return; p.removeChild(node); rebuildContent(); }
    ...
    function treeGroupAdd(node, tag) {
        var p = node.parentNode; if (!p) return; var newEl;
        var defs = { li: 'Новый пункт', dt: 'Термин', dd: 'Описание', p: 'Новый абзац', blockquote: 'Цитата' };
        if (tag === 'tr') { newEl = node.cloneNode(true); newEl.removeAttribute(CFG.MARK); Array.prototype.forEach.call(newEl.querySelectorAll('[' + CFG.MARK + ']'), function(x) { x.removeAttribute(CFG.MARK); }); Array.prototype.forEach.call(newEl.querySelectorAll('td,th'), function(c) { c.innerHTML = ''; }); }
        else if (defs[tag]) { newEl = S.doc.createElement(tag); newEl.textContent = defs[tag]; }
        else if (tag === 'section' || tag === 'article') { newEl = node.cloneNode(true); newEl.removeAttribute(CFG.MARK); Array.prototype.forEach.call(newEl.querySelectorAll('[' + CFG.MARK + ']'), function(x) { x.removeAttribute(CFG.MARK); }); }
        else return;
    ...
    function buildTreeView2(container) { container.style.padding = '4px 6px 24px'; renderTreeChildren(S.doc.body, container, 0); }

    /* ════════ РЕЖИМ: ВИЗУАЛЬНЫЙ ════════ */
    var VCSS = [
        '*,*:before,*:after{box-sizing:border-box;}html,body{margin:0;padding:0;}',
        'body{font-family:"Helvetica Neue",Arial,Tahoma,sans-serif;font-size:14px;line-height:1.65;color:#333;padding:16px 20px;background:#fff;}',
        ...
        'img{max-width:100%;height:auto;vertical-align:middle;}a{color:#3076be;text-decoration:underline;}',
        'table{border-collapse:collapse;width:100%;}th,td{padding:4px 8px;border:1px solid #ddd;text-align:left;}',
        ...
        '[data-sev8]{outline:2px dashed rgba(48,118,190,.45);outline-offset:1px;cursor:pointer;border-radius:2px;transition:outline-color .12s,background .12s;}',
    ...
    function buildVisual() {
        ...
        wrap.appendChild(mk('div', { cls: 'sev8-bar info', html: '<span class="sev8-bar-txt">✏ Кликайте на подсвеченный элемент. <strong>Ссылки заблокированы.</strong></span>' }));
    ...
    function renderVFrame(fr) {
        ...
        iDoc.open();
        iDoc.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><style>' + VCSS + '</style></head><body>' + (S.doc ? S.doc.body.innerHTML : '') + '</body></html>');
        iDoc.close();
    ...
    function bindVFrame(fr) {
        ...
        iDoc.addEventListener('click', function(e) {
            ...
            while (cur && cur !== iDoc.body) { if (cur.getAttribute && cur.getAttribute(CFG.MARK)) { marked = cur; break; } cur = cur.parentElement; }
    ...
    function openEdit(fr, iEl, id) {
        ...
        Array.prototype.forEach.call(iDoc.querySelectorAll('.sev8-active'), function(e) { e.classList.remove('sev8-active'); });
        iEl.classList.add('sev8-active');
        if (info.type === 'text' && /^h[1-6]$/.test(info.tag) && !iEl.querySelector('[' + CFG.MARK + ']')) {
            var orig = iEl.innerHTML; iEl.contentEditable = 'true'; iEl.focus();
            iEl.onblur = function() {
                iEl.contentEditable = 'false'; iEl.classList.remove('sev8-active');
                info.node.innerHTML = iEl.innerHTML;
                syncNow();
            };
    ...
    function showOvl(fr, iEl, info, id) {
        ...
        var top = frRect.top + iRect.top, left = frRect.left + iRect.left;
        var ovlW = Math.min(Math.max(iRect.width + 24, 340), 560);
        left = Math.max(10, Math.min(left, W.innerWidth - ovlW - 10));
        top = Math.max(10, Math.min(top, W.innerHeight - 340));
        var ovl = mk('div', { id: 'sev8-ovl', css: 'top:' + top + 'px;left:' + left + 'px;width:' + ovlW + 'px;' });
        var hdr = mk('div', { id: 'sev8-ovl-hdr' });
        hdr.appendChild(mk('span', { id: 'sev8-ovl-title', txt: '✏ <' + info.tag + '>' }));
        var xBtn = mk('button', { cls: 'sev8-btn', type: 'button', txt: '✕' }); xBtn.onclick = closeOvl;
        ...
        if ((info.type === 'attrs' || info.type === 'both') && info.attrs) {
            info.attrs.forEach(function(attr) {
                if (!info.node.hasAttribute(attr)) return;
                var row = mk('div', { id: 'sev8-ovl-atrow' });
                row.appendChild(mk('span', { id: 'sev8-ovl-atlbl', txt: attr + ':' }));
                var ai = mk('input', { type: 'text', cls: 'sev8-in', css: 'font-family:monospace;font-size:11px;flex:1;min-width:0;margin-bottom:0;', value: info.node.getAttribute(attr) || '' });
                attrInps[attr] = ai; row.appendChild(ai); ovl.appendChild(row);
            });
        }
        var btns = mk('div', { id: 'sev8-ovl-btns' });
        var cancelBtn = mk('button', { cls: 'sev8-btn', type: 'button', txt: 'Отмена' }); cancelBtn.onclick = closeOvl;
        ...
        saveBtn.onclick = function() {
            ...
            Object.keys(attrInps).forEach(function(a) { info.node.setAttribute(a, attrInps[a].value); iEl.setAttribute(a, attrInps[a].value); });
            closeOvl(); syncNow();
        };
    ...
    function closeOvl() {
        ...
        if (S.curIframe) { try { var iDoc = S.curIframe.contentDocument || S.curIframe.contentWindow.document; Array.prototype.forEach.call(iDoc.querySelectorAll('.sev8-active'), function(e) { e.classList.remove('sev8-active'); }); } catch(e) {} }
    }
    ...
    function buildSource() {
        ...
        bar.appendChild(mk('span', { cls: 'sev8-bar-txt', html: '⚠ Исходный HTML. <b>Ctrl+S</b> / <b>Ctrl+Enter</b> — применить.' }));
        ...
        function updateStatus() { srcStatus.textContent = ta.value.split('\n').length + ' стр. / ' + ta.value.length + ' симв.'; }
        updateStatus();
        function applySource() {
            var newHtml = ta.value; if (newHtml === S.rawHtml) return;
            S.rawHtml = newHtml; parseAndMark(S.rawHtml); writeHTML(S.rawHtml); histPush(S.rawHtml);
            setStatus('Исходник применён ✓', 2500);
        }
        ...
        ta.addEventListener('keydown', function(e) {
            if ((e.ctrlKey || e.metaKey) && (e.key === 's' || e.key === 'Enter')) { e.preventDefault(); applySource(); setStatus('Применено ✓', 2000); }
            if (e.key === 'Tab') { e.preventDefault(); var start = ta.selectionStart, end = ta.selectionEnd; ta.value = ta.value.substring(0, start) + '  ' + ta.value.substring(end); ta.selectionStart = ta.selectionEnd = start + 2; }
        });
        ta.addEventListener('blur', function() { if (ta.value !== S.rawHtml) applySource(); });
        taWrap.appendChild(ta); taWrap.appendChild(srcStatus); wrap.appendChild(taWrap); return wrap;
    }

    /* ════════ HTML ФОРМАТТЕР ════════ */
    function fmtHTML(src) {
        var TAB = '  ';
        var BLOCK = /^(div|section|article|header|footer|main|nav|aside|p|ul|ol|li|table|thead|tbody|tfoot|tr|caption|blockquote|figure|figcaption|h[1-6]|form|fieldset|dl|dt|dd|address|details|summary|dialog)$/;
        ...
        function serAttrs(el) { var s = ''; Array.prototype.forEach.call(el.attributes, function(a) { s += ' ' + a.name + '="' + a.value.replace(/&/g, '&amp;').replace(/"/g, '&quot;') + '"'; }); return s; }
        function hasBlockChild(el) { var r = false; Array.prototype.forEach.call(el.childNodes, function(c) { if (c.nodeType === 1 && BLOCK.test(c.tagName.toLowerCase())) r = true; }); return r; }
        function ser(node, depth) {
            var out = '';
            Array.prototype.forEach.call(node.childNodes, function(c) {
                var pad = TAB.repeat(Math.max(0, depth));
                if (c.nodeType === 8) { out += pad + '<!--' + c.nodeValue + '-->\n'; }
                else if (c.nodeType === 3) { var txt = c.nodeValue.replace(/\s+/g, ' ').trim(); if (txt) out += pad + txt + '\n'; }
                else if (c.nodeType === 1) {
                    var tag = c.tagName.toLowerCase(), attrs = serAttrs(c);
                    if (VOID.test(tag)) out += pad + '<' + tag + attrs + '>\n';
                    else if (RAW.test(tag)) out += pad + '<' + tag + attrs + '>' + c.innerHTML + '</' + tag + '>\n';
                    else if (BLOCK.test(tag)) {
                        if (!hasBlockChild(c) && c.innerHTML.trim()) out += pad + '<' + tag + attrs + '>' + c.innerHTML.replace(/\s+/g, ' ').trim() + '</' + tag + '>\n';
                        else if (!c.innerHTML.trim()) out += pad + '<' + tag + attrs + '></' + tag + '>\n';
                        else { out += pad + '<' + tag + attrs + '>\n'; out += ser(c, depth + 1); out += pad + '</' + tag + '>\n'; }
                    } else out += pad + c.outerHTML + '\n';
                }
            });
            return out;
        }
        ...
        var doc;
        try { doc = (new DOMParser()).parseFromString('<!DOCTYPE html><html><head></head><body>' + safe + '</body></html>', 'text/html'); }
        catch(e) { S.tokens = savedTokens; return src; }
        var result = ser(doc.body, 0).replace(/\n{3,}/g, '\n\n').trim();
        var restored = detokenize(result);
        S.tokens = savedTokens; return restored;
    }
    ...
    function mount(rawHtml) {
        ...
        /* Скрываем нативный редактор */
        if (S.aceMode && S.aceEl) {
            S.aceEl.style.display = 'none';
        } else if (S.nativeTa) {
            S.nativeTa.style.display = 'none';
        }

        /* Контейнер для вставки редактора */
        var insertTarget = null;
        if (S.aceMode && S.aceEl) {
            insertTarget = S.aceEl.parentElement;
        } else if (S.nativeTa) {
            insertTarget = S.nativeTa.parentElement;
        }
        if (!insertTarget) {
            L('ERR: не найден контейнер для вставки редактора');
            return;
        }
        ...
        if (S.aceMode && S.aceEl) {
            insertTarget.insertBefore(S.rootEl, S.aceEl);
        } else if (S.nativeTa) {
            insertTarget.insertBefore(S.rootEl, S.nativeTa);
        }
    ...
    if (W.Ext) {
        ...
        setTimeout(function() {
            if (!S.mounted) {
                L('ExtJS не обнаружен, прямой запуск...');
                startPolling();
            }
        }, 15000);
    }
